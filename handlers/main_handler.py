...
from database.queries import load_today_to_db
...
async def get_conclusion(message: Message, state: FSMContext, conn:AsyncConnection):
    data = await state.get_data()
    await load_today_to_db(conn, data['mood'], data['day_desc'], data['mentor_desc'],message.text)
    await message.answer('Спасибо за твой отчет! Надеюсь ты вынес из этого свои выводы. Этот отчет сохарнился, ты можешь посмотреть его в лююое время')
    await state.clear()